   ...
   case SQL_INVALID_HANDLE: 
    {
     info = false;
     ret = false;
     errors.push_back(MySqlError(1, 0, L"Invalid Handle"));
    } break;
   ...
   while(loop==true)
    {
     ...
     if (lrc==SQL_SUCCESS)
      ...
      {
       if (loop == true)
         errors.push_back(MySqlError(iRec, wszState, L"Failed to get SQL error"));
       loop=false;
      }
    }
  }
 ...
 switch(m_SQLType)
  {
   ...
   case SQL_C_TIMESTAMP:
     ...
     break;     
   case SQL_C_BINARY: 
     m_BinaryLength = (SQLLEN) m_Len;
     rc=::SQLBindParameter(hStmt, col, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_BINARY, (SQLULEN)m_Len, 0, (SQLPOINTER)m_ByteVal, m_Len, &m_BinaryLength);
     break;
   default: 
...
void MySqlCommand::ExecuteNonQuery(bool bIgnoreError)
{
 ...
 if (m_hStatement == 0)
   throw L"m_hStatement not established, probably default constructor";
